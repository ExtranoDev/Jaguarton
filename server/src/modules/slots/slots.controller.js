...
const generate = asyncHandler(async (req, res) => {
  ...
  const slots = await slotsService.generateSlots(req.params.id, req.user.id, {
    date,
    startHour,
    endHour,
    durationMinutes,
...
const updateStatus = asyncHandler(async (req, res) => {
  ...
  const slot = await slotsService.setSlotBlocked(req.params.id, req.user.id, blocked);
  res.status(200).json({ slot });
});

const remove = asyncHandler(async (req, res) => {
  await slotsService.deleteSlot(req.params.id, req.user.id);
  res.status(204).send();
});
